...
# Тест 11: Создание питомца без имени
def test_creating_a_pet_without_a_name(name='', animal_type='манул', age='2', pet_photo='images/photo3.jpg'):
    pet_photo = os.path.join(os.path.dirname(__file__), pet_photo)
    _, auth_key = pf.get_api_key(valid_email, valid_password)
    status, result = pf.add_new_pet(auth_key, name, animal_type, age, pet_photo)
    assert status == 200
...
# Тест 12. Создание питомца без возраста
def test_creating_a_pet_without_age(name='Блэк', animal_type='доберман', age='', pet_photo='images/photo1.jpg'):
    pet_photo = os.path.join(os.path.dirname(__file__), pet_photo)
    _, auth_key = pf.get_api_key(valid_email, valid_password)
    status, result = pf.add_new_pet(auth_key, name, animal_type, age, pet_photo)
    assert status == 200
